...
export function GameFour() {
  ...
  // Create a ref for the punch sound
  const punchSoundRef = useRef(null);

  useEffect(() => {
    punchSoundRef.current = new Audio("/sounds/playerPunch.mp3");
  }, []);
  ...
  const movePlayer = (key, playerIndex) => {
    ...
    if (grids[newY][newX].canPass && !isCellOccupied(newX, newY, playerIndex)) {
      ...
      player.isHurt = false; // Clear the hurt state when moving
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 text-gray-500">
        Click the screen to control. Use WASD to move the red player. Use Arrow
        Keys to punch. Smack other players!
      </p>
        ...
        <div className="absolute inset-0 bg-black bg-opacity-75 flex justify-center items-center text-white text-4xl">
          {typeof winner === "number" ? (
            ...
            <div className="flex flex-col">
              <button
                onClick={startGame}
                className="px-6 py-3 bg-blue-500 rounded-lg shadow-lg hover:bg-blue-600 cursor-pointer"
              >
                Start Game
              </button>
              <button
                onClick={() => navigate("/")}
                className="px-6 py-3 self-center w-fit bg-gray-500 rounded-lg shadow-lg hover:bg-gray-600 cursor-pointer mt-4"
              >
                Back to Lobby?
              </button>
            </div>
      ...
      {gameOver && (
        <div className="absolute inset-0 bg-black bg-opacity-75 flex flex-col justify-center items-center text-white text-4xl">
          <div className={`mb-4 text-${winner}-200`}>{winner} wins!</div>
          <button
            onClick={restartGame}
